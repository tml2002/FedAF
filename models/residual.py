...
class Residual(torch.nn.Module):
    ...
    def extract_weight_names(self, model):
        watermarked_layer_names = []
        for name, param in model.named_parameters():
            if 'weight' in name and name in self.m_layers:
                # 假设 self.m_layers 是包含水印的层的名称列表
                watermarked_layer_names.append(name)
        return watermarked_layer_names
